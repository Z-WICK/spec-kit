...
- **Codex CLI compatibility improvements**
  - Updated `specify init` next steps to show Codex-native command invocation: `/prompts:speckit.*`
  - Made `CODEX_HOME` setup optional and clarified fallback behavior when using default Codex prompt directories
  - Updated enhanced command hints to use Codex-native prompt invocation when `--ai codex` is selected

- **Documentation updates**
  - Clarified Codex setup and usage in README (`.codex/prompts/` location and `/prompts:speckit.*` invocation)
