...
scripts:
  flyway:
    ...
    ps: .specify/scripts/powershell/scan-flyway-versions.ps1
  alembic:
    sh: .specify/scripts/bash/scan-alembic-revisions.sh
    ps: .specify/scripts/powershell/scan-alembic-revisions.ps1
  clarify_guard:
    sh: .specify/scripts/bash/clarify-requirement-guard.sh
    ps: .specify/scripts/powershell/clarify-requirement-guard.ps1
---
...
This shared pipeline template is agent-agnostic across slash-command and skill-based CLIs.
It is not a hook-equivalent wrapper around the core specify -> plan -> tasks -> implement
workflow; in skill-based agents such as Codex it does not automatically execute
command-level hooks declared in `.specify/extensions.yml`.

### Load Project Config

Read `.specify/.project` as a plain `KEY=VALUE` config file generated by the project
initialization workflow.
Do **not** use `source`, `eval`, variable expansion, or command substitution.
...
- Ignore unknown keys.
- Treat values as literal strings only.
- Do not expand `$VAR`, `${VAR}`, or `$(...)`.

Command safety rules for `BUILD_COMMAND`, `TEST_COMMAND`, `DEPLOY_COMMAND`, and `SERVICE_LOG_COMMAND`:
- Only allow one command plus arguments.
- Reject shell composition or redirection syntax, including: `;`, `&&`, `||`, `|`, `>`, `>>`, `<`, `` ` ``, `$()`.
- If validation fails, stop the current stage immediately and report the unsafe command.
- If a project needs a complex workflow, require the command to be wrapped in a repo-controlled script first.

Validation rules for `BASE_BRANCH`:
- Treat it as a literal git ref name, not shell text.
- Reject whitespace and shell control syntax.
- Accept only values that pass strict git ref validation before use in any `git diff`, `git checkout`, `git merge`, or `git push` command.
- If validation fails, stop the current stage immediately and ask the user to provide a safe branch name.

If `.specify/.project` does not exist, warn the user:
```
WARNING: .specify/.project not found. Run the project initialization workflow first to detect project config.
```
...
If gate script is missing, stop and tell the user to run the project update workflow first
so the latest pipeline assets are installed.
...
3. Run gate script (must pass):
   - Bash:
     ```bash
     ./.specify/scripts/bash/pipeline-stage-gate.sh --stage N --feature-dir "<FEATURE_DIR>" --worktree-root "<WORKTREE_ROOT>" --docs-summary-file "<RUNTIME_DIR>/docs-summary.md" --main-repo-root "<main-repo-root>" --base-branch "<validated BASE_BRANCH>" --receipt "<RUNTIME_DIR>/stage-N.receipt.json"
     ```
   - PowerShell:
     ```powershell
     ./.specify/scripts/powershell/pipeline-stage-gate.ps1 -Stage N -FeatureDir "<FEATURE_DIR>" -WorktreeRoot "<WORKTREE_ROOT>" -DocsSummaryFile "<RUNTIME_DIR>/docs-summary.md" -MainRepoRoot "<main-repo-root>" -BaseBranch "<validated BASE_BRANCH>" -Receipt "<RUNTIME_DIR>/stage-N.receipt.json"
     ```
...
| 8 | `<FEATURE_DIR>/merge-summary.md` exists and merge to `<validated BASE_BRANCH>` recorded |
...
**Checkpoint**:
- Confirm `<FEATURE_DIR>/impact-pre-analysis.md` exists and is non-empty.
- If CRITICAL downstream risks found, append them as warnings to `plan.md` and ensure tasks generation accounts for them (e.g., add adaptation tasks for affected downstream modules).
...
Proceed to task generation? (yes/no)
  ...
  no  - Stop here; you can review artifacts first, then re-run
        this pipeline command to auto-resume from stage 4
============================================================
```
...
(multi-module), validate `BUILD_COMMAND` with the command safety rules above, then run it as one literal command plus arguments from within `"<WORKTREE_ROOT>"`.

Example:

```bash
cd "<WORKTREE_ROOT>" && <validated BUILD_COMMAND>
```
...
Task:
  ...
  prompt: |
    ...
    1. Run: git diff <validated BASE_BRANCH>..HEAD --stat  (to get changed file list)
    2. Run: git diff <validated BASE_BRANCH>..HEAD          (to get full diff)
...
Task:
  ...
  prompt: |
    ...
    Review all changes on this branch relative to <validated BASE_BRANCH>:
    1. Run git diff <validated BASE_BRANCH>..HEAD for the full diff
...
- **CRITICAL/HIGH issues exist** -> show issue list, ask user:
  ...
  Auto-fix? (yes/no)
    yes  - Dispatch each issue to coding-worker for fixing,
           then re-run review
    no   - Stop; you can fix manually then re-run this pipeline command
  ```
...
Before dispatch, validate `TEST_COMMAND` with the command safety rules above.
The test-runner agent takes the exact validated command (from `.specify/.project`), runs it,
parses the output, and returns only a structured pass/fail summary. It does NOT detect
the project type — that was already done during the initialization workflow.
...
After tests pass, auto-merge to `<validated BASE_BRANCH>`:
...
Merge the feature branch to `<validated BASE_BRANCH>` (cross-platform git command):

```bash
cd <main-repo-root> && git checkout <validated BASE_BRANCH> && git merge <BRANCH_NAME> --no-ff -m "feat: merge <BRANCH_NAME> with tests"
```
...
```
============================================================
Branch <BRANCH_NAME> merged to <validated BASE_BRANCH> (local).
  Tests: all passed (N test classes, M test methods)
  Review: PASS

Push to remote? (yes/no)
  yes  - git push origin <validated BASE_BRANCH>
  no   - Keep local merge; you can push manually
============================================================
```

- User selects yes -> `git push origin <validated BASE_BRANCH>`
...
Validate `DEPLOY_COMMAND` with the command safety rules above before execution.

```bash
cd "<main-repo-root>" && <validated DEPLOY_COMMAND>
```
...
Validate `SERVICE_LOG_COMMAND` with the same command safety rules above before dispatching `log-analyzer`.
...
Pipeline complete:
  ...
  Merge: <merged to <validated BASE_BRANCH> / pending>
